/**
 * Claw Codex Prompt Fetcher
 *
 * Fetches and caches the codex.txt system prompt from Claw's GitHub repository.
...
const CLAW_CODEX_URL =
	"https://raw.githubusercontent.com/anthropics/claw/dev/packages/claw/src/session/prompt/codex.txt";
const CACHE_DIR = join(homedir(), ".claw", "cache");
const CACHE_FILE = join(CACHE_DIR, "claw-codex.txt");
const CACHE_META_FILE = join(CACHE_DIR, "claw-codex-meta.json");
...
/**
 * Fetch Claw's codex.txt prompt with ETag-based caching
 * Uses HTTP conditional requests to efficiently check for updates
 *
 * Rate limit protection: Only checks GitHub if cache is older than 15 minutes
 * @returns The codex.txt content
 */
export async function getClawCodexPrompt(): Promise<string> {
	...
	try {
		const response = await fetch(CLAW_CODEX_URL, { headers });
		...
		throw new Error(`Failed to fetch Claw codex.txt: ${response.status}`);
	} catch (error) {
		...
		throw new Error(
			`Failed to fetch Claw codex.txt and no cache available: ${error}`
		);
	}
}

/**
 * Get first N characters of the cached Claw prompt for verification
